#!/bin/bash
# Re-sign every pointer record against the delegate this source currently builds.
#
# Run this whenever the delegate WASM changes, BEFORE committing, then commit
# pointer-records.toml alongside the change that caused it. CI
# (check-pointer-freshness) fails the PR if you forget.
#
...
#   scripts/sign-pointer-records.sh --no-build # trust target/ as-is (see below)
#
# The key is read from ~/.config/ghostkeys/web-container-keys.toml — the same
# key that signs the vault webapp, which Makefile.toml already names as
# KEY_FILE. It is piped straight into the signer and never lands in a variable,
# a temp file, or argv.
#
# ghostkeys has no other publisher identity: ghost keys are per-USER blind
# signed certificates, so there is no pre-existing "ghostkeys author" key. See
# pointer-records.toml.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
TOML_PATH="pointer-records.toml"
KEY_FILE="${GHOSTKEYS_POINTER_KEY_FILE:-$HOME/.config/ghostkeys/web-container-keys.toml}"
...
for arg in "$@"; do
    case "$arg" in
        ...
        *) die "unknown argument '$arg' (expected --force and/or --no-build)" ;;
    esac
done
...
# whole trust anchor for every integrator resolving a ghostkeys pointer.
#
...
# ghostkeys commits no WASM, so the bytes being signed are BUILT here. Through
# the script, never a bare `cargo build`: the registry path remap that keeps the
# delegate key machine-independent lives there, and without it this would sign
# a hash nobody else can reproduce (ghostkeys#9).
if [ "$BUILD" -eq 1 ]; then
    echo "Building the delegate (scripts/build-delegate.sh) ..."
    bash scripts/build-delegate.sh >/dev/null || die "the delegate failed to build; refusing to sign a stale artifact"
fi
...
for i in $(seq 1 "$N"); do
    ...
    [ -f "$WASM_PATH" ] || die "$WASM_PATH not found (record $i, $APP_ID). Build it: bash scripts/build-delegate.sh"
    NEW_HASH="$(b3sum --no-names "$WASM_PATH")"
    ...
    # release was ignored.
    #
    # An empty OLD_VERSION would make this arithmetic fail under `set -e`
    # rather than silently produce version 1 over an existing record.
    [ -n "$OLD_VERSION" ] || die "record $i ($APP_ID) has no version"
...
if [ "$CHANGED" -eq 0 ]; then
    echo ""
    echo "Nothing to do — every record already names the delegate this source builds."
    exit 0
fi

echo ""
echo "Re-verifying through the gate CI will run..."
# --no-build: the delegate was built above (or deliberately not), and rebuilding
# here would hide a --no-build caller's stale target/ from themselves.
./scripts/check-pointer-freshness.sh --no-build
