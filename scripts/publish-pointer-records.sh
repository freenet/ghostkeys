#!/bin/bash
# Publish ghostkeys' signed pointer records to the Freenet network.
#
...
WEB_CONTAINER_KEY="DLog47hEsrtuGT4N5XCeMBG45m4n1aWM89tBZXue2E1N"
GH_REPO="freenet/ghostkeys"
...
# separately checked against the artifact live on the network. Refusing on
# untracked files blocked every publish in the Atlas port (freenet/atlas#47),
# whose checkout normally carries build artifacts — and this repo's checkout
# ALWAYS does, because the delegate WASM under target/ is a build output.
#
...
# exactly the provenance claim being made.
...
RUNS="$(gh run list --repo "$GH_REPO" --commit "$HEAD_SHA" --json conclusion,status,name 2>/dev/null || echo "")"
...
if [ "$BAD" != "0" ]; then
    gh run list --repo "$GH_REPO" --commit "$HEAD_SHA" --limit 20 | sed 's/^/    /'
...
# bytes is 16 characters about 18% of the time. A `{17}` pattern therefore
# undercounts always, and returns ZERO for a node whose peers all happen to be
# short — which would abort every publish claiming the node has no peers.
# Requiring the following column separator keeps the 46/47-character contract
# keys in the same output from matching.
...
# The pointer WASM must be the COMMITTED upstream artifact, never a local
# rebuild. A locally rebuilt WASM lands every record at a key nobody else
# derives — invisible to every consumer, and indistinguishable from success
# here.
echo ""
...
from freenet-migrate (contracts/pointer-contract/pointer-v1.wasm).
...
but $TOML_PATH names
  $POINTER_CODE_HASH
Publishing this would put every record at an address nobody derives.
...
#
# The record must name the delegate users are actually running, not the one
# this checkout builds. Those diverge in practice — merging a delegate change
# is not the same event as republishing the vault.
#
# ghostkeys does NOT ship the delegate WASM as a bundle file (the UI embeds it
# via include_bytes!), so unlike Delta there is no live WASM to re-hash. What
# the live bundle does carry is delegate-key.json, written at publish time from
# the very bytes being published (scripts/write-delegate-pointer.sh) and gated
# by scripts/check-delegate-pointer.sh. So that file IS the network's statement
# of which delegate is live, and it is what the record is checked against.
#
# The check is therefore one link longer than Delta's, and that link is worth
# naming: it trusts delegate-key.json rather than re-deriving from live bytes.
# The publish-time gate is what makes it trustworthy, so if that gate is ever
# removed, this check degrades with it.
echo ""
echo "[2] the code hash in the record == the delegate LIVE ON THE NETWORK"
say "fetching the live vault web container ($WEB_CONTAINER_KEY) ..."
...
PY
tar -xJOf "$WORK/webapp.tar.xz" delegate-key.json > "$WORK/live-delegate-key.json" 2>/dev/null \
    || die "could not read delegate-key.json from the live webapp archive.
Refusing to publish a record whose target could not be checked against the network."
LIVE_HASH="$(python3 -c "import json,sys; print(json.load(open(sys.argv[1]))['code_hash'])" "$WORK/live-delegate-key.json")"
LIVE_KEY="$(python3 -c "import json,sys; print(json.load(open(sys.argv[1]))['delegate_key'])" "$WORK/live-delegate-key.json")"
# delegate-key.json carries both, and they are redundant by construction
# (key = BLAKE3(code_hash bytes), empty params). Checking the redundancy costs
# nothing and catches a bundle whose two fields were written from different
# builds — the exact drift check-delegate-pointer.sh exists to prevent, so
# a disagreement here means that gate was bypassed.
DERIVED_LIVE_KEY="$(printf '%s' "$LIVE_HASH" | xxd -r -p | b3sum --no-names)"
[ "$DERIVED_LIVE_KEY" = "$LIVE_KEY" ] || die "the LIVE delegate-key.json is internally inconsistent:
  code_hash      $LIVE_HASH
  derives to     $DERIVED_LIVE_KEY
  but names key  $LIVE_KEY
Refusing to publish against a bundle whose own two fields disagree."
...
but the delegate LIVE on the network is $LIVE_HASH.

This is the divergence that matters: publishing this record would point every
integrator at a delegate that is not what users are running, and they would
find an empty namespace. Republish the vault first (cargo make
publish-ghostkeys), or re-sign against the live bytes."
    fi
...
Refusing to publish over state whose provenance is unclear."
        ...
        # situations: re-running after a partial publish, and two delegate
        # changes merging before anyone runs the network publish.
...
#
# These used to be two loops upstream. Under `set -e` a failed `fdev` on record
# 2 killed the script before the verification loop ran at all — including for
# record 1, which had already gone live. Re-reading is the only thing that
# proves a publish worked; running it in a second loop meant it was skipped in
# exactly the partial-failure case where it matters most.
#
...
echo ""
echo "An integrator resolving these now gets the code hash of the delegate that"
echo "is actually live. Addressing only — say nothing about ghost keys surviving."
